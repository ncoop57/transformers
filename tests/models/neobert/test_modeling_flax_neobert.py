...
from transformers import NeoBERTConfig, is_flax_available, is_tokenizers_available
...
if is_flax_available():
    ...
    from transformers.models.neobert.modeling_flax_neobert import FlaxNeoBERTForCausalLM, FlaxNeoBERTModel
...
@require_flax
class FlaxNeoBERTModelTest(FlaxModelTesterMixin, FlaxGenerationTesterMixin, unittest.TestCase):
    all_model_classes = (FlaxNeoBERTModel, FlaxNeoBERTForCausalLM) if is_flax_available() else ()
    all_generative_model_classes = (FlaxNeoBERTForCausalLM,) if is_flax_available() else ()

    def setUp(self):
        self.model_tester = FlaxNeoBERTModelTester(self)
    ...
    def test_model_from_pretrained(self):
        for model_class_name in self.all_model_classes:
            model = model_class_name.from_pretrained("openlm-research/open_neobert_3b_v2", from_pt=True)
...
@slow
@require_flax
class FlaxNeoBERTIntegrationTest(unittest.TestCase):
    def setUp(self):
        self.model_id = "openlm-research/open_neobert_3b_v2"
        self.model = FlaxNeoBERTForCausalLM.from_pretrained(self.model_id, from_pt=True)
